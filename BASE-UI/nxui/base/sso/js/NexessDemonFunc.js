...
document.write("<script type='text/javascript' charset='utf-8' src='js/jquery-3.6.0.min.js'></script>");
document.write("<script type='text/javascript' charset='utf-8' src='js/jquery-3.6.0.js'></script>");

// 환경 설정
var CONST_LOCAL_DEMON_ADDRESS = "https://127.0.0.1:17489";  // 포트는 NCConfig.ini의 [NCDemon] 섹션의 DPort 항목에 설정된 포트 값과 동일해야 함.
...
var CONST_SERVER_NC_VERSION = "1.0.0.0"
...
// 아래로 NC 제어 함수
function Nexess_ByPassLogin(callback, encrypted_id, time)
{
    Print_ConsoleLog(">> Nexess_ByPassLogin Func Call");

    var reqData = new requestData();
    Init_RequestData(reqData);

    reqData.func = "ByPassLogin";
    reqData.param = encrypted_id + "|" + time;
...
function Nexess_Execute_SAP(callback, external_id, SAP_parameter)
{
    Print_ConsoleLog(">> Nexess_Execute_SAP Func Call");

    var reqData = new requestData();
    Init_RequestData(reqData);

    reqData.func = "Execute_SAP";
    reqData.param = external_id + "|" + SAP_parameter;
    reqData.callback = callback;
...
function Nexess_GetTime_YYYYMMDDHHMMSS()
{
    Print_ConsoleLog(">> Nexess_GetTime_YYYYMMDDHHMMSS Func Call");

    var time_now = new Date();

    var year = time_now.getFullYear().toString();
    var month = (time_now.getMonth() + 1) >= 10 ? (time_now.getMonth() + 1).toString() : '0' + (time_now.getMonth() + 1).toString();
    var date = time_now.getDate() >= 10 ? time_now.getDate().toString() : '0' + time_now.getDate().toString();
    var hours = time_now.getHours() >= 10 ? time_now.getHours().toString() : '0' + time_now.getHours().toString();
    var minutes = time_now.getMinutes() >= 10 ? time_now.getMinutes().toString() : '0' + time_now.getMinutes().toString();
    var seconds = time_now.getSeconds() >= 10 ? time_now.getSeconds().toString() : '0' + time_now.getSeconds().toString();
...
function Nexess_GetServerConnect(callback)
{
    Print_ConsoleLog(">> Nexess_GetServerConnect Func Call");

    var reqData = new requestData();
    Init_RequestData(reqData);

    reqData.func = "GetServerConnect";
    reqData.param = "";
    reqData.callback = callback;
...
function Nexess_SetServerConnect(callback, server_type)
{
    Print_ConsoleLog(">> Nexess_SetServerConnect Func Call");

    var reqData = new requestData();
    Init_RequestData(reqData);

    reqData.func = "SetServerConnect";
    reqData.param = server_type;
    reqData.callback = callback;
...
function Nexess_ExecuteSAPShotCutForHanmi(callback, gui_parm, system_name, number, lang)
{
    Print_ConsoleLog(">> Nexess_ExecuteSAPShotCutForHanmi Func Call");

    var reqData = new requestData();
    Init_RequestData(reqData);

    reqData.func = "ExecuteSAPShotCutForHanmi";
    reqData.param = gui_parm + "|" + system_name + "|" + number + "|" + lang;
    reqData.callback = callback;
...
function Nexess_GetExtentionalField(callback, user_id, field_name)
{
    Print_ConsoleLog(">> Nexess_GetExtentionalField Func Call");

    var reqData = new requestData();
    Init_RequestData(reqData);

    reqData.func = "GetExtentionalField";
    reqData.param = user_id + "|" + field_name;
    reqData.callback = callback;
...
function Nexess_GetConnectNLSURL(callback, encrypted_id, time)
{
