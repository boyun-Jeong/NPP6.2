...
if (!nexacro.DesignForm)
{
    ...
    nexacro.loadStringResource = function (lang, url)
    {
        ...
        if (app)
        {
            app._load_manager.reloadJsonModule(stringrc_url, null, false, service);
   ...
   // 	trace("_pDesignForm.getCurrentStyleValue(\"" + compid + "\")" + " propid:" + propid);
    ...
    _pDesignForm._getHorizontalCreateRectByPoint = function (x, y, rootcompid, recursive)
    {
            ...
            {                
                if (gap_left > 0 && gap_right > 0) {
                    if (gap_left > gap_right) {
                        var this_idx = comp._getForm()._p_components.indexOf(this._getScopeName(right_comp));
                        var pre_comp = comp._getForm()._p_components[this_idx - 1];
                        if (pre_comp) {
        ...
        else
        {
            ...
            if (!recursive && parent && parent._p_form == rootobj._p_form)
            {
            ...
            {
                var comps = comp._is_group ? comp._group_panel._getComponents() : comp._getForm()._p_components;
                var this_idx = comps.indexOf(this._getScopeName(comp));
                var pre_comp = comps[this_idx - 1];
                if (pre_comp) 
                {
            ...
            {                
                if (gap_top > 0 && gap_bottom > 0)
                {
                    if (gap_top > gap_bottom) {
                        var this_idx = comp._getForm()._p_components.indexOf(this._getScopeName(bottom_comp));
                        var pre_comp = comp._getForm()._p_components[this_idx - 1];
                        if (pre_comp) {
                            var rect = this._getClientRect(pre_comp);
                ...
                {
                    var this_idx = comp._getForm()._p_components.indexOf(this._getScopeName(comp));
                    var pre_comp = comp._getForm()._p_components[this_idx - 1];
                    if (pre_comp) {
                        rect = this._getClientRect(pre_comp);
                        return [rect[0], rect[1] + rect[3], rect[2], width];
  ...
  //  	trace("DesignForm.showPreviewContents:" + compid);
    ...
    _pDesignForm.loadStringResource = function (languagecode)
    {
        // TODO : Design화면 language 변경        
        nexacro.loadStringResource(languagecode);
    };
    ...
    _pApplicationAccessPort.loadStringResource = function (languagecode) {
        nexacro.loadStringResource(languagecode);
    };
