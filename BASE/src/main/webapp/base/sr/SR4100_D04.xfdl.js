(function()
{
    return function()
    {
        ...
        {
            this.set_name("SR4100_D4");
            this.set_titletext("처리결과");
        ...
        // User Script
        this.addIncludeScript("SR4100_D04.xfdl","lib::lib_WorkFlow.xjs");
        this.registerScript("SR4100_D04.xfdl", function() {
        /***********************************************************************************
        * 화면 ID	︰ SR4100_D4
        * 화면(명)	︰ 처리결과
        * 메뉴(경로)	︰
        ...
        ***********************************************************************************/
        ...
        this.fnMsgAfter = function(mID,varValue)
        {
        	...
        	switch(mID)
        	{
        		case "calPlanSdt" :		//처리시작일
        			this.divContent.form.calPlanSdt.setFocusRed();
        			break;

        		case "mskPlanStime" :	//처리시작시간
        			this.divContent.form.mskPlanStime.setFocusRed();
        			break;

        		case "calPlanEdt" :		//처리종료일
        			this.divContent.form.calPlanSdt.setFocusRed();
        			break;

        		case "mskPlanEtime" :	//처리종료시간
        	...
        	if(this.parent.editYn == "Y")
        	{
        		...
        		if(this.reqInfo.WF_PROC_CD == "WF_PROC_2030")
        		{
        			if(selecToday <  nexacro.toNumber(Ex.util.today()))
        			{
        				var param = {
        					  id	: "calPlanSdt"
        					, msg	: "처리시작일은 오늘날짜보다 크거나 같아야합니다."	//메시지 내용
        			...
        			else if( Ex.isEmpty(this.dsWfPbm.getColumn(0,"PROC_SDT")) )
        			{
        				var param = {
        					  id	: "calPlanSdt"
        					, msg	: "처리시작일을 확인하시기 바랍니다."	//메시지 내용
        					, arrparam : []						//메시지의 변수에 들어갈 실제값
        					, msgtype : "E"						//메시지 타입 '' : [No icon]아이콘 없는 기본 alert, 'I' : [Info]일반 alert, 'W' : [Warning]경고 Alert, 'S' : [Success]성공 alert(저장 등에 사용)&#13;
        				};
        				Ex.core.alert(this, param);
        			...
        			if(selecEndday <  selecToday)
        			{
        				var param = {
        					  id	: "calPlanEdt"
        					, msg	: "처리완료일은 처리시작일보다 크거나 같아야합니다."	//메시지 내용
        					, arrparam : []						//메시지의 변수에 들어갈 실제값
        					, msgtype : "E"						//메시지 타입 '' : [No icon]아이콘 없는 기본 alert, 'I' : [Info]일반 alert, 'W' : [Warning]경고 Alert, 'S' : [Success]성공 alert(저장 등에 사용)&#13;
        				};
        				Ex.core.alert(this, param);
        			...
        			if( Ex.isEmpty(this.dsWfPbm.getColumn(0,"PROC_EDT")) )
        			{
        				var param = {
        					  id	: "calPlanEdt"
        ...
        this.loadIncludeScript("SR4100_D04.xfdl");
