...
if (!nexacro.ComplexComponent) {
	...
	_pComplexComponent._getNCChildren = function () {
		...
		if (this._is_nc_scroll) {
			...
			if (this.hscrollbar && this.hscrollbar._p_visible) {
				children.push(this.hscrollbar);
			}
		}
		if (this._is_nc_expand) {
			if (this.expandbar && this.expandbar._p_visible) {
	...
	_pComplexComponent._checkScrollControl = function (before) {
		...
		return (hscrollbar && hscrollbar._p_visible && hscrollbar._height) << 1 | (vscrollbar && vscrollbar._p_visible && vscrollbar._width);
	};
